...
nodeB = Node(2, 200,'Black',[]);
nodeC = Node(3, 200,'Black',[]);
nodeD = Node(4, 200,'Black',[]);
...
NodeSize = [1 0.5 0.5 0.5]; % Коэффициенты 
...
rng(123);
% Генерация данных
timeRange = [1000, 11000];      
TinfRange = [900, 1200];          

% Случайные значения времени и температуры окружающей среды (значения X)
timeValues = rand(numSamples, 1) * (timeRange(2) - timeRange(1)) + timeRange(1);
TinfValues = rand(numSamples, 1) * (TinfRange(2) - TinfRange(1)) + TinfRange(1);
...
for i = 1:numSamples
    inputParams = [timeValues(i); TinfValues(i)];
    TavgValues(i) = HeatBC.CalcCoreFunction(inputParams) - 75;
end
...
for i = 1:numSamples
    for j = 1:numOfNodes
        inputParams = [timeValues(i); TinfValues(i)];
        XData(i) = XData(i).addRow(inputParams);
    end
end
...
%% Настройка
trainer.Train(XDataTrain, YDataTrain, XDataTest, YDataTest, 0.1, 0.9, 0.99,1e-8, NodeSize, NodeWeight, 100, 1e7, -1e7, 6.0, 0.5, 0.1, [], 'mae');
%% Тестирование
...
disp('Первые 3 временные:');
timeValues(1:3)
disp('Последняя временная');
timeValues(500)

disp('Первые 3 температуры:');
TinfValues(1:3)
disp('Последняя температура');
TinfValues(500)

disp('Первые 3 результирующих:');
TavgValues(1:3)
disp('Последний результирующий');
TavgValues(500)
...
% Генерация данных
timeRange = [1000, 15000];      
TinfRange = [800, 1200];          

% Случайные значения времени и температуры окружающей среды (значения X)
timeValues = rand(validSamples, 1) * (timeRange(2) - timeRange(1)) + timeRange(1);
TinfValues = rand(validSamples, 1) * (TinfRange(2) - TinfRange(1)) + TinfRange(1);

% Зададим синтетические значения средней температуры, смещенной
% относительно нагрева соседних вершин
TavgValues = zeros(validSamples, 1);
...
for i = 1:validSamples
    for j = 1:numOfNodes
        inputParams = [timeValues(i); TinfValues(i)];
        XDataValid(i) = XDataValid(i).addRow(inputParams);
    end
end
